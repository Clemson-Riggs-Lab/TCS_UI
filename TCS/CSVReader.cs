...
public class CSVReader
{
    ...
    public List<Cue> Load(string csvfile,bool timed=false,DateTime startTime= default(DateTime))
    {
        ...
        string[][] grid = CsvParser2.Parse(text);
            ...
            int.TryParse(grid[i][8], out row.StartingTactorLocation);
            int.TryParse(grid[i][9], out row.MiddleTactorLocation);
            int.TryParse(grid[i][10], out row.EndingTactorLocation);
            int.TryParse(grid[i][11], out row.StartingISI);
            int.TryParse(grid[i][12], out row.EndingISI);
            int.TryParse(grid[i][13], out row.StartingPulseDuration);
            int.TryParse(grid[i][14], out row.EndingPulseDuration);
            int.TryParse(grid[i][15], out row.StartChangeAfterPulseNumber);
            int.TryParse(grid[i][16], out row.EndChangeAfterPulseNumber);
            int.TryParse(grid[i][17], out row.DelayWithin1);
            int.TryParse(grid[i][18], out row.DelayBetween1);
            int.TryParse(grid[i][19], out row.DelayWithin2);
            int.TryParse(grid[i][20], out row.DelayBetween2);
